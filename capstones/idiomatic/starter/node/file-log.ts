...
export class NodeLogStorage implements LogStorage {
  ...
  readText(): Promise<string | undefined> {
    return Promise.reject(incomplete("TODO(m3-adapter): read Node event logs"));
  }

  createText(_text: string): Promise<void> {
    return Promise.reject(incomplete("TODO(m3-adapter): create Node event logs"));
  }

  appendText(_text: string): Promise<void> {
    return Promise.reject(incomplete("TODO(m3-adapter): append Node event logs"));
  }
